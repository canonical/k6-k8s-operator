...
def _wait_for_idle(juju, timeout=300, poll_interval=10):
...
def _assert_prometheus_has_k6_metrics(juju, retries=30, delay=10):
...
def test_deploy_prometheus(juju, charm_path):
...
def test_scale_up_and_run_multi_unit(juju):
